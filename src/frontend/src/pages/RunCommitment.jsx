...
export default function RunCommitment({ apiBase }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const r = await fetch(`${apiBase}/api/run/protocol`, {
        ...
        body: JSON.stringify({ protocol: 'commitment', backend }),
      })
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-slate-100">Commitment</h1>
      <p className="text-sm text-slate-500">
        Run commitment protocol on simulation or IBM hardware. Hardware runs return a job_id and stream status via WebSocket.
      </p>
